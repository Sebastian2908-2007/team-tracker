...
const {writeTeam, copyCss} = require('../utils/write-copy');
let managerArr = [];
let engineerArr = [];
let internArr = [];
 ...
 // passes info to the htmlgen "aka" template.js
  teamGen() {
      inquirer.prompt({
          type: 'confirm',
          name:'confirmBuild',
          message: 'are you ready to generate you team?',
          default: true
    }).then(confirm => {
        if(confirm.confirmBuild === true) {
            return htmlGen(managerArr,engineerArr,internArr);
        }
        throw this.chooseEmployee();
    }).then(teamHtml => {
        return writeTeam(teamHtml);
